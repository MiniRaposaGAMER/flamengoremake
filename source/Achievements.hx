...
class Achievements {
	public static function init()
	{
		...
		createAchievement('week1_nomiss',			{name: "Uma Vez Flamengo", description: "Beat Flamengo Week on Hard with no Misses."});
	...
	public static function reloadList()
	{
		...
		final modLoaded:String = Paths.currentModDirectory;
		Paths.currentModDirectory = null;
		loadAchievementJson(Paths.mods('data/achievements.json'));

		for (i => mod in ModsMenuState.parseList().enabled)
		{
			final bitch:String = Paths.mods('$mod/data/achievements.json');
			if (sys.FileSystem.exists(Paths.mods(bitch))) {
				Paths.currentModDirectory = mod;
				loadAchievementJson(bitch);
			}
		}
	...
	inline static function loadAchievementJson(path:String, addMods:Bool = true)
	{
		...
		if(!sys.FileSystem.exists(path)) return retVal;
	...
	public static function addLuaCallbacks(lua:State)
	{
		Lua_helper.add_callback(lua, "getAchievementScore", function(name:String):Float
		{
			if(!achievements.exists(name))
			{
				psychlua.LuaUtils.luaTrace(lua, 'getAchievementScore: Couldnt find achievement: $name', false, false, FlxColor.RED);
				return -1;
			}
		...
		Lua_helper.add_callback(lua, "setAchievementScore", function(name:String, ?value:Float = 1, ?saveIfNotUnlocked:Bool = true):Float
		{
			if(!achievements.exists(name))
			{
				psychlua.LuaUtils.luaTrace(lua, 'setAchievementScore: Couldnt find achievement: $name', false, false, FlxColor.RED);
				return -1;
			}
		...
		Lua_helper.add_callback(lua, "addAchievementScore", function(name:String, ?value:Float = 1, ?saveIfNotUnlocked:Bool = true):Float
		{
			if(!achievements.exists(name))
			{
				psychlua.LuaUtils.luaTrace(lua, 'addAchievementScore: Couldnt find achievement: $name', false, false, FlxColor.RED);
				return -1;
			}
		...
		Lua_helper.add_callback(lua, "unlockAchievement", function(name:String):Dynamic
		{
			if(!achievements.exists(name))
			{
				psychlua.LuaUtils.luaTrace(lua, 'unlockAchievement: Couldnt find achievement: $name', false, false, FlxColor.RED);
				return null;
			}
			return unlock(name);
		});
		Lua_helper.add_callback(lua, "isAchievementUnlocked", function(name:String):Dynamic
		{
			if(!achievements.exists(name))
			{
				psychlua.LuaUtils.luaTrace(lua, 'isAchievementUnlocked: Couldnt find achievement: $name', false, false, FlxColor.RED);
				return null;
			}
